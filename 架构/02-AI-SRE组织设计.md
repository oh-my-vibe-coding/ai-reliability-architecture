...
[第 11 章](01-AI系统参考架构.md) 给了一张组件图，但组件归谁负责、谁向谁交付什么、谁参加哪个例会——这些在图上看不到。这一章把组件图转成**组织图**：给每个组件配团队、给每条接缝定接口、给每个反复出事故的边界写明确的 RACI。
...
按工程师规模有三档典型组织。每一档都对应 [第 11 章](01-AI系统参考架构.md) 的某一档实现，但不是 1:1 锁定——组织规模可能领先或落后于系统档位。
...
- 在 [第 11 章](01-AI系统参考架构.md) 的 **S 档**
- 单一上游、单一业务场景
- 不需要专门的红队 / 合规岗位

致命陷阱：

- **不要试图复制大公司的 RACI 矩阵**——50 人公司搞 7 个团队的接口契约，开会比写代码多
- **不要把"AI SRE"写成专职岗 JD**——会招不到合适的人，且来了也只能接零散活
...
| 团队 | 主管组件（[第 11 章](01-AI系统参考架构.md) 命名）| 团队规模 | 主要 KPI |
|---|---|---|---|
...
应用团队不在这张图里——它们是"客户"（消费平台的服务，但不是甲乙方，见 §4 边界 5），按业务线划分，使用上面三个团队提供的平台。

适用条件：

- 在 [第 11 章](01-AI系统参考架构.md) 的 **M 档**或正在跨向 L 档
- ≥ 3 条业务线在用 LLM
- 月度推理花费 > $100k（2026-06 标定，随 [第 11 章标定表](01-AI系统参考架构.md) 同步重标）
...
- 在 [第 11 章](01-AI系统参考架构.md) 的 **L 档**
- ≥ 10 条业务线在用 LLM
- 单年推理花费 > $50M（2026-06 标定，随 [第 11 章标定表](01-AI系统参考架构.md) 同步重标），或核心场景受合规约束
- 已有专职安全 / 合规部门
...
| 事故 IC（数据泄漏 · 技术处置）| C | C | I | C | **R/A** | I |
| 数据泄漏披露与监管通报 | I | I | I | I | C | **R/A** |
...
最常见的三种错配（Eval 归 ML、网关不看质量、红队挂在 ML 下）见 §6 反模式 6 / 3 / 5。

### 把 A 从"岗位"升成"可追责的资格"

上面这张表里的 A，目前还停在**岗位**这一层——"平台 SRE 签字""法务签字"。对大多数行这够用。但有一类决策，岗位级的 A 不够：**高 blast-radius、且不可逆的自动化决策**——L3/L4 自治放行、不可逆动作的双签、把某条核心链路交给 Agent 自动执行（见 [架构决策 3 · Agent 自治边界](03-架构师的决策框架.md) 与 [第 15 章](05-不可逆决策与Day2状态.md)）。

对这类决策，A 不能只是"某个岗位"，必须落到一个**具名、有资格门槛、可追责**的人。这一步把工程史上反复出现的做法搬过来：锅炉年代是第三方检验师签字、电气年代是注册工程师（PE）对设计签字担责、ASME 让具名工程师为压力容器背书——靠的都不是"某个部门负责"，而是"某个**够格的人**签了字，出事找他"。

一个够格的签字人至少满足四条：

- **具名到人**，不是"由 X 团队负责"——出事时翻得到是谁签的。
- **过了能力门槛**：亲手跑过这条链路的红队与 eval，能说清它的失败模式和 blast radius，而不是只看了一页汇报。
- **亲自背复盘责任**：这条决策出事，复盘会上署名的是他，而不是"团队"。
- **有权说"不"**：能在业务压力下顶回一个"先上了再说"，且这个"不"不会被他的汇报线轻易推翻——这正是前面强调横切团队**独立汇报线**的原因。

落到 RACI 上：凡是涉及**不可逆 + 高 blast-radius** 的行，A 这一格不能填团队名，要填一个满足上述资格的具名角色，并在决策记录里留下签字人。这就是把"在场的人"从一句职责口号，变成一条可追责的制度——也是 [结语](../99-结语.md) 那张"制度性回应"表里，AI 时代留给我们自己填的那一格。
...
**修复**：每个核心团队的 KPI 里必须有**至少一个跨界指标**。网关团队 KPI 含"按 trace 抽样的引用支持率"；Eval 团队 KPI 含"端到端 p99"。这种"看似不归我管"的指标才能推动跨团队对话——网关团队有了质量指标，才有动力做影子流量、canary 探针。

### 反模式 4 · Prompt 没有版本所有者

Prompt 散在 git 各处，每个应用一个 readme，没有人对"Prompt Registry 的健康度"负责。某天发现 prompt 静默改动半年没人审，已经回不去了。

**修复**：Prompt Registry 必须有一名维护者（通常在平台 SRE 或 ML 平台团队），这名维护者对"prompt 版本回滚 < 5 分钟"这个 SLO 负责。Registry 的建法、变更管道与回滚前提见 [深入 21](../深入/21-Prompt作为运维对象.md)。

### 反模式 5 · 红队挂在 ML 团队下

ML 团队的 KPI 是模型上线速度，红队是它的成本中心——结果红队总被砍预算或被合并到"安全周"。攻击者不会按你的日历来。

**修复**：红队必须有独立汇报线（向安全 / 质量 / 法务汇报均可，但不向 ML 汇报），至少有一名常驻人员，并维护持续运行的对抗样本库——每次模型 / prompt 上线触发自动化红队回归。

### 反模式 6 · "Eval 归 ML"

Eval 团队向 ML VP 汇报，ML VP 的 KPI 是模型上线速度——Eval 的实际作用变成"帮 ML 拿绿灯"，而不是"帮组织把住质量门"，等于让被考试的人自己出题。

**修复**：Eval 必须独立于 ML 汇报——ML 提供 rubric，但不掌控 gold set。模式 B 推荐向 Quality 或 SRE 汇报；模式 C 推荐独立 Quality VP。

### 反模式 7 · FinOps 不参与 AI 设计评审

财务发现 AI 账单一个月烧了 $5M 时已经晚了——架构里早就有"每个用户请求平均 50k token"这种问题。

**修复**：月推理花费 ≥ $1M（2026-06 标定，随 [第 11 章标定表](01-AI系统参考架构.md) 同步重标）的组织，FinOps 必须出席 AI 系统的架构评审。详见 [第 16 章 · 预算治理](06-预算治理.md) 和 [第 17 章 · 与外部世界的契约](07-与外部世界的契约.md)。
...
- **不是个人能力建模**——AI SRE 个人画像看 [理念 02](../理念/02-SRE架构师的角色迁移.md) 和 [第 9 章](../知识/08-组织与判断力.md)
- **不是组织变革管理**——怎么推动组织接受这套 RACI 是另一门学问，本章只给"应然"

## 接下来

- **下一章**：[第 13 章 · 架构师的决策框架](03-架构师的决策框架.md) —— 6 类高频决策的判据矩阵
- **配套**：[第 9 章 · 组织与判断力](../知识/08-组织与判断力.md) —— 个人层面的角色与能力
...
[← 第 11 章 · AI 系统参考架构](01-AI系统参考架构.md)  ·  下一章 → [第 13 章 · 架构师的决策框架](03-架构师的决策框架.md)
