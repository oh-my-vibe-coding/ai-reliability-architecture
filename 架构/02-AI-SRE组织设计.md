...
[第 10 章](01-AI系统参考架构.md) 给了一张组件图，但组件归谁负责、谁向谁交付什么、谁参加哪个例会——这些在图上看不到。这一章把组件图转成**组织图**：给每个组件配团队、给每条接缝定接口、给每个反复出事故的边界写明确的 RACI。
...
按工程师规模有三档典型组织。每一档都对应 [第 10 章](01-AI系统参考架构.md) 的某一档实现，但不是 1:1 锁定——组织规模可能领先或落后于系统档位。
...
| 团队 | 主管组件（[第 10 章](01-AI系统参考架构.md) 命名）| 团队规模 | 主要 KPI |
|---|---|---|---|
...
- 在 [第 10 章](01-AI系统参考架构.md) 的 **M 档**或正在跨向 L 档
- ≥ 3 条业务线在用 LLM
- 月度推理花费 > $100k（2026-06 标定，随 [第 10 章标定表](01-AI系统参考架构.md) 同步重标）
...
- 在 [第 10 章](01-AI系统参考架构.md) 的 **L 档**
- ≥ 10 条业务线在用 LLM
- 单年推理花费 > $50M（2026-06 标定，随 [第 10 章标定表](01-AI系统参考架构.md) 同步重标），或核心场景受合规约束
- 已有专职安全 / 合规部门
...
上面这张表里的 A，目前还停在**岗位**这一层——"平台 SRE 签字""法务签字"。对大多数行这够用。但有一类决策，岗位级的 A 不够：**高 blast-radius、且不可逆的自动化决策**——L3/L4 自治放行、不可逆动作的双签、把某条核心链路交给 Agent 自动执行（见 [架构决策 3 · Agent 自治边界](03-架构师的决策框架.md) 与 [第 14 章](05-不可逆决策与Day2状态.md)）。
...
**修复**：月推理花费 ≥ $1M（2026-06 标定，随 [第 10 章标定表](01-AI系统参考架构.md) 同步重标）的组织，FinOps 必须出席 AI 系统的架构评审。详见 [第 15 章 · 预算治理](06-预算治理.md) 和 [第 16 章 · 与外部世界的契约](07-与外部世界的契约.md)。
...
- **下一章**：[第 12 章 · 架构师的决策框架](03-架构师的决策框架.md) —— 6 类高频决策的判据矩阵
...
[← 第 10 章 · AI 系统参考架构](01-AI系统参考架构.md)  ·  下一章 → [第 12 章 · 架构师的决策框架](03-架构师的决策框架.md)
