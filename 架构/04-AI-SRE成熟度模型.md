---
title: 第 13 章 · AI SRE 成熟度模型（L1-L5）
updated: 2026-07-04
tags: [part-3, architecture, maturity-model]
---

# 第 13 章 · AI SRE 成熟度模型（L1-L5）

> 所属：第三部分 · 架构决策  ·  [← 返回目录](../README.md)

[第 10 章](01-AI系统参考架构.md) 的 S/M/L 是**系统形态**——讲组件配齐了几档。本章的 L1-L5 是**控制能力**——讲组织在多大程度上能驾驭它建出来的系统。两者不一定锁定：S 档可能 L3，L 档也可能假装 L4 实际 L2。这一章给一套判据，让架构师每季度问一次"我们到底处在哪一级、缺什么、卡在哪"。
...
先消一个歧：本书到这里已经出现四把分级尺子——系统形态 S/M/L（[第 10 章](01-AI系统参考架构.md)）、Agent 自治 L0-L4（[第 12 章](03-架构师的决策框架.md) 决策 3）、Eval 投入档位 E1-E4（第 12 章决策 4）、成熟度 L1-L5（本章）。它们量的不是同一个东西，互不换算——本章往下的 Lx 一律指成熟度。
...
- **它把组织能力 / 系统能力 / 控制能力分开**——一个团队可能系统看上去很强但控制能力很弱（[第 10 章](01-AI系统参考架构.md) L 档但本章 L2）
...
| 级别 | 一句话 | 系统对应 | 组织档位（[第 11 章](02-AI-SRE组织设计.md)）|
|---|---|---|---|
...
- 月度推理花费 > $5k（2026-06 标定，随 [第 10 章标定表](01-AI系统参考架构.md) 同步重标）——开始有财务压力
- 团队决定把这个用例**正式纳入产品**
...
| **组织** | 有 [第 11 章](02-AI-SRE组织设计.md) 模式 A 的最小 RACI；有人能接事故 |
...
- 业务线 ≥ 3 条都在用 LLM
- 月度推理花费跨过 [第 10 章 S 档上界](01-AI系统参考架构.md)
- 出现端到端事故无人能 IC（接入网关团队的关键时机，见 [第 11 章 反模式 1](02-AI-SRE组织设计.md)）
- 单一上游故障导致业务级事故
...
| **成本** | 按业务线 / 客户分摊；预算告警（[第 15 章 · 预算治理](06-预算治理.md)）|
| **组织** | [第 11 章 模式 B 三角组织](02-AI-SRE组织设计.md) 完整；端到端 SLO 有单一负责人 |
...
- 进入 [第 10 章 L 档](01-AI系统参考架构.md) 的触发条件已满足
- 有合规审计要求（金融 / 医疗 / 政府）
- 出现过隐蔽的静默降级事故，发现"L3 的 eval 不够覆盖"
- 月度推理花费跨过 [第 10 章 L 档下界](01-AI系统参考架构.md) 且增速持续 > 30%/年
...
| **安全** | 致命三角三腿全控；红队独立汇报线（[第 11 章 反模式 5](02-AI-SRE组织设计.md)：独立汇报线不依赖矩阵组织）；隐私保护（差分隐私、PII 自动脱敏）|
| **成本** | 单位经济性（unit economics）建模；Token / GPU / Risk / Error budget 联合治理（[第 15 章](06-预算治理.md)）|
| **组织** | 模式 B 完整 + 横切职能独立：Trust & Safety 独立汇报线、FinOps 进入设计评审；仅当满足 [第 11 章 B→C 触发条件](02-AI-SRE组织设计.md) 时才引入矩阵 |
...
- [ ] Token / GPU / Risk / Error budget 有联合视图（[第 15 章](06-预算治理.md)）
- [ ] FinOps 出席 AI 设计评审

### 组织（6 项）

- [ ] 端到端用户感知 SLO 有单一负责人团队
- [ ] [第 11 章 RACI](02-AI-SRE组织设计.md) 中关键决策每行都有 R 且不重复
- [ ] Eval 团队的汇报线独立于 ML
- [ ] Prompt Registry 有专人维护，回滚 < 5 分钟（[深入 21](../深入/21-Prompt作为运维对象.md)）
- [ ] 法务 / 合规 / 安全的对外契约清单有定期更新（[第 16 章](07-与外部世界的契约.md)）
...
flowchart TB
    ...
    L1 -.对应.-> A1S[第 10 章 · S 档]
    L2 -.对应.-> A1S
    L3 -.对应.-> A1M[第 10 章 · M 档]
    L4 -.对应.-> A1ML[第 10 章 · M 后期 / L 早期]
    L5 -.对应.-> A1L[第 10 章 · L 档稳定]

    L2 -.推动.-> A2A[第 11 章 · 模式 A]
    L3 -.推动.-> A2B[第 11 章 · 模式 B]
    L4 -.推动.-> A2B2[第 11 章 · 模式 B 成熟<br/>横切独立汇报线]
    L5 -.推动.-> A2C[第 11 章 · 模式 C]

    L3 -.判据基础.-> A3[第 12 章 · 决策框架]
    L4 -.前提条件.-> A5[第 14 章 · 不可逆]
    L4 -.前提条件.-> A6[第 15 章 · 预算治理]
    L4 -.前提条件.-> A7[第 16 章 · 外部契约]
```

简单读：

- 看[第 10 章](01-AI系统参考架构.md) 能知道**系统该长什么样**
- 看[第 11 章](02-AI-SRE组织设计.md) 能知道**组织该长什么样**
- 看[第 12 章](03-架构师的决策框架.md) 能知道**单次决策怎么做**
...
- **下一章**：[第 14 章 · 不可逆决策与 Day 2 状态](05-不可逆决策与Day2状态.md)
...
[← 第 12 章 · 架构师的决策框架](03-架构师的决策框架.md)  ·  下一章 → [第 14 章 · 不可逆决策与 Day 2 状态](05-不可逆决策与Day2状态.md)
