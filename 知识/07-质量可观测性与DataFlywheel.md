---
title: 第 8 章 · 质量可观测性与 Data Flywheel
updated: 2026-07-02
tags: [part-2, knowledge]
---

# 第 8 章 · 质量可观测性与 Data Flywheel
...
本章的核心洞察——也是 Hamel Husain 的 eval 方法论、Anthropic 的事故复盘等一系列实践里逐渐收敛出的共识：
...
- **延迟**：eval 结果多久能回到 dashboard？（分钟级足以驱动告警，慢到小时级就只剩看板价值——目标值见深入 06 的 eval SLO 表）
- **Judge κ**：judge 模型与人工标注的一致性系数（Cohen's κ，一种修正了偶然一致的统计量——κ=1 表示完全一致，κ=0 表示和随机猜一样，0.4–0.6 算中度一致、0.6 以上进入高度一致区间；生产 judge 通常要求 ≥0.6），周度跟踪
- **Pipeline uptime**：eval 管线自己挂了多久？eval 挂 = 质量盲眼
- **失败率**：eval 作业本身有多少比例因为各种原因失败

这些指标之上还要配一条 error budget policy——eval 亏预算时同样要触发行动。这也是为什么不能把 eval 推给"测试团队"——它是**生产观测基础设施**，归 SRE 管才合逻辑。
...
- **下一章**：[第 9 章 · 组织与判断力](08-组织与判断力.md) —— 前面几章都在讲"方法论要换"，这一章反过来：有些能力 AI 换不掉，价值反而更高
- **深入专题**（遇到具体问题再回查）：
...
上一章 → [第 7 章 · 从链到图：多智能体系统的可靠性](从链到图-多智能体系统的可靠性.md)
下一章 → [第 9 章 · 组织与判断力](08-组织与判断力.md)
