...
>
> 这就是量化里 outlier feature 的直觉。模型几千个维度中，某几个维度值在 ±150，其余全在 ±1 以内。为了让那几个 ±150 能表示，整个量化的刻度被"撑大"了，±1 以内的值全部挤到几个粗糙的刻度——精度几乎为零。那个 168cm 被量成 160 的同学，误差 8cm——但只有遇到"需要精确身高判断"的任务（比如衣服尺码推荐）才会暴露，测平均身高看不出来。那个 172cm 被量成 176 的同学误差只有 4cm，看起来"还行"——但刻度间距已经被撑到 16cm，他的精度同样没了，只是这次恰好落得近，暂时没触发。
...
> [!WARNING] 何时这条不再适用
> 这张表建立在 2026-05 的硬件与量化方法现状上。当以下任一事件发生，请重测而不是照抄：
>
> - 硬件原生 FP8/FP4（NVFP4、MXFP4）在你的分类 eval 上追平或超过 int4 PTQ → "AWQ int4 当前 PTQ 最佳质量"不再成立，首选项换成硬件原生格式；
> - 你所用推理引擎的默认量化路径发生切换（默认格式、默认 KV cache 精度换代）→ GGUF 档位、FP8 KV cache 这类生态推荐要跟着重审。
