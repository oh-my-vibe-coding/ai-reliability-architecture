...
- **Prompt 是代码**。它需要版本、评审、回放、灰度和回滚（详见 [深入 21 · Prompt 作为运维对象](21-Prompt作为运维对象.md)）。
...
> [!WARNING]
> 很多 AI 系统不是死在第 1 层，而是死在第 2-4 层：服务全绿，但引用错、工具错、用户不信、回滚慢。
...
- 每类任务的 L1 assertion 是什么（三层 eval 正篇见 [深入 06](06-Eval-Pipeline设计.md)，速查版在 [深入 03](03-模型与工具场景化最佳实践.md) §2.5）？
- L2 judge 有没有和人工校准？
...
- 要做质量闭环：读 [第 8 章](../知识/07-质量可观测性与DataFlywheel.md) 和 [深入 06](06-Eval-Pipeline设计.md)
