...
这是 **SRE 架构师该拥有的基础设施**，而不是推给"ML 团队"。它是可靠性反馈回路。每一圈的 Owner / 频率 / 触发条件 / 失败处理四件事在[第 7 章](../知识/07-质量可观测性与DataFlywheel.md)定义，本章不重复。
...
排障顺序也照这个来：final response 失败 → 看 trajectory 定位偏离点 → 用 single step eval 分析那一步的决策。轨迹数据就是你的 trace（[第 7 章](../知识/07-质量可观测性与DataFlywheel.md)），eval 和可观测性在这里合流。
