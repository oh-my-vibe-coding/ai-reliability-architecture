...
- 拿厂商模型 + 厂商微调 API（OpenAI / Google Vertex 有；Anthropic 第一方没有，仅 Bedrock 上的旧款 Claude 3 Haiku 可 SFT）→ 上传数据 → 拿到自己的模型 ID
- 拿基础模型 → 全参微调 → 自己跑推理

这三条路在 **SRE 视角下**有完全不同的运维负担——但本书前 13 篇深入专题都默认"模型是别人给的、你只负责推理"，对这一块着墨严重不足。本章补这个缺口。
...
- OpenAI / Google Vertex AI 提供自助 fine-tuning API；Anthropic 第一方 API 不提供微调，唯一通道是 AWS Bedrock 上的旧款 Claude 3 Haiku SFT（2026-07 快照 🕒，以厂商文档为准）
...
> [!NOTE]
> **上面三种是按"算力 / 参数量"分的形态；训练目标是另一条正交轴**。本篇（连同全书其余部分）默认目标是 **SFT**（监督微调：喂标注样本让模型模仿）。到 2026，另一条已产品化的路径是 **RFT（reinforcement fine-tuning）**——用一个可校验的奖励 / grader（而非标注样本）来训练，擅长带可判定结果的 agent / 推理 / 代码任务，OpenAI / Fireworks / Bedrock 均有自助入口。对 SRE 来说关键是：RFT 的运维负担与 SFT **同构**——一样要版本化、一样在**基座升级后必须重训 / 重测**、一样进 [Model Registry](15-模型注册表与上线流程.md)。所以下文的生命周期、回滚、成本框架对 SFT 与 RFT 都适用，差异主要在"训练信号从标注样本换成了奖励函数"。
...
| **去重** | 同一条数据出现 N 次会让模型记住而不是学习（见 [共同语言 02 · Data 是 ML 的真正核心](../共同语言/02-Data是ML的真正核心.md) + [代码 06 MinHash 去重](../代码/06-dedup-minhash.py)）|
...
- AWS Bedrock 模型微调（含 Claude 3 Haiku）— https://docs.aws.amazon.com/bedrock/latest/userguide/custom-model-fine-tuning.html
