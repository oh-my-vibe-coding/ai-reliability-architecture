...
> [← 返回目录](../README.md)  ·  对应知识章节：[第 5 章 · AI 推理服务的可靠性工程](../知识/05-AI推理服务的可靠性工程.md)  ·  相关：[深入 10 · 事故模式库](10-AI系统事故模式库.md)、[深入 14 · 微调作为运维对象](14-微调作为运维对象.md)、[深入 21 · Prompt 作为运维对象](21-Prompt作为运维对象.md)
...
| **base-model** | Llama 3.3 / Qwen 3 / DeepSeek V4 等**开放权重**模型 | sha256 验证、license 追踪（见 §2.1）|
...
### 2.1 base-model 的 license 追踪：追什么

上表给 base-model 写了"license 追踪"，但**追踪不等于抄一个许可证名字进 YAML**。这里的 base-model 几乎都是[**开放权重**](../附录/C-术语表.md)模型——权重能下载、能自托管、能微调，训练数据和训练代码通常不公开。日常统称的"开源模型"绝大多数是这一段，而**"开放权重"和"开源"在许可证层面差别很大**：都能下载，但能不能商用、能不能拿输出去训别的模型、派生模型要不要改名，是四类完全不同的约束。

对 SRE 的意义很直接：这些约束**不是法务部门的事后审查项，而是上线前的阻塞门**。等产品上线一年后才发现许可证不允许当前用法，整改手段只剩"换基座重训"——那是季度级工程量，且触发 [第 15 章](../架构/05-不可逆决策与Day2状态.md) 的不可逆决策。

按**约束形状**分四类（这层结构长寿，具体型号归哪类会随版本变）：

| 类别 | 约束形状 | 上线前必须回答 |
|---|---|---|
| **宽松许可**（Apache-2.0 / MIT 等）| 商用、修改、再分发基本自由，通常只要求保留版权声明 | 归档 LICENSE 文件即可，风险最低 |
| **厂商自有社区许可**（各家 "community license"）| 可商用，但**附条件**：常见的是用户规模阈值（超过即需单独授权）、派生模型命名要求、可接受使用政策（AUP）| 我们当前规模离阈值有多远？派生模型名字合规吗？ |
| **限制性 / 研究许可** | 禁止商用，或禁止用模型输出去训练其他模型（"禁蒸馏"条款）| 我们是不是正拿它的输出造训练集？（**这条最容易被无意违反**）|
| **权重待发 / 分阶段放出** | 已发 API、但权重尚未放出或只放部分档位 | 自建方案能不能等？有没有回退到 API 的路径？ |

要落进注册表 YAML 的字段，是**可判定的事实**而不是许可证全文：

```yaml
# model-registry/qwen3-7b-instruct.yaml（字段示意）
license:
  spdx: Apache-2.0           # 无标准 SPDX id 的自有许可证写 LicenseRef-<厂商>-<名称>
  url: https://...           # 官方 LICENSE 文件的 permalink，不是博客或第三方转述
  retrieved: 2026-07-25      # 取回日期——许可证会随版本改
  commercial_use: true
  derivative_naming: null    # 有命名要求就写清要求原文
  output_training_allowed: true   # 能否用输出训练其他模型
  user_threshold: null       # 阈值型条款写清阈值与当前水位
  reviewed_by: legal-2026-07  # 谁审的、什么时候审的
```

三条实践规则：

- **以官方 repo 的 LICENSE 文件为准**，不采信第三方聚合站或模型卡摘要——同一模型不同档位（base / instruct / 不同尺寸）许可证可能不同，第三方常混为一谈。本书正文提到的具体型号许可证同样只是快照，选型前自行复核（当期快照见 [深入 03 · 第三部分](03-模型与工具场景化最佳实践.md)）。
- **许可证随版本变，不随模型名变**。升级 base model 的 patch 版本要重新取回 LICENSE，别假设 `v3.3` 和 `v3.1` 同许可——这和 [深入 14 · §5.3](14-微调作为运维对象.md) 的"base 升级即所有 LoRA 重测"是同一类"以为没变其实变了"。
- **权重开放 ≠ 可审计**。合规审计问"这条用户数据训练过哪些模型"时，开放权重基座只能答到"线上跑的是哪个权重 sha256"——训练数据不在你手上。这条边界要在审计口径里写明，别承诺答不到的层级（§1.2 列的三个 SRE 诉求里，第三条对 base-model 只能部分满足）。
...
quality_gates:
  ...
  general_regression:
    max_drop: 0.02            # 回退幅度上限：通用任务最多掉 2%（正值表示掉多少）
  safety:
...
| [深入 21](21-Prompt作为运维对象.md) | 姊妹篇：模型工件归本章，prompt 工件归深入 21，"四联版本绑定"在两章间闭环 |
