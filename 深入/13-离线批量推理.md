...
| 厂商 | 接口 | 折扣 | 单批上限 | 完成时间 SLA |
|---|---|---|---|---|
| Anthropic Message Batches | `/v1/messages/batches` | **50%** | 10 万条 / 256MB | < 24h（多数 < 1h）|
| OpenAI Batch API | `/v1/batches` | **50%** | 5 万条 / 200MB | < 24h |
| Google Vertex AI Batch | `BatchPredictionJob` | ~50% | — | < 24h |
| DeepSeek 折扣时段 | 16:30-00:30 UTC | **75%** | —（非 batch 接口）| 实时但限流 |

> 价格 / 时段 / 单批上限为 2026-07 快照，使用前查官方。

**SRE 工作**：
- 选用 Batch API 而不是循环调标准 endpoint —— **直接省一半钱**
- 按单批条数 / 体积上限做分批切片与提交编排（1M 条按 5-10 万条/批的上限是 10-20 个 batch，不是一个）
- 监控 batch job 状态（pending / in_progress / completed / failed）
- 把"24h SLA"作为业务依赖告警的输入
- 失败 batch 要有重提交流程（**不能手工**，必须自动）

> [!TIP]
> **常见错误**：把"50% 折扣"理解为"少花一半钱就行"，结果用标准 endpoint 跑 100 万次循环调用。Batch API 同样的 1M 请求按单批上限拆成 10-20 个 batch、每批一次提交——调用次数从 100 万降到 10-20 次，网络、限流、错误处理仍简化多个数量级。
...
- 选 Qwen3-8B（开源 + 中文好 + 摘要任务足够）
- 单 H100 跑 Qwen3-8B offline batch：按 ~25k tokens/s 聚合吞吐估（输入 + 输出合计；这种 10:1 输入输出比的大 batch 负载由 prefill 主导。量级估算，落地前用自己的负载实测校准）
- 1 亿条 × 550 token = 55B token 总流量
- 单卡：55B / 25k = 2.2M 秒 ≈ **25 卡天**
- 24h 完成需要 **~26 张 H100** 全程不停
- AWS spot H100 ~$2/h（on-demand 一半价）→ 26 × 24 × 2 ≈ **$1.2k**
...
| 自建 vLLM + spot | ~$1.2k + 1 周搭建 | 摊薄到月度后极低 | 长期 / 高频时碾压 |
