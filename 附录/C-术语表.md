...
| 开放权重（Open-Weight） | **权重文件可下载、可自托管、可微调，但训练数据与训练代码通常不公开——所以它不等于"开源"**。模型可得性是一条三段光谱：**闭源 API**（只给一个 endpoint）→ **开放权重**（给权重文件，多附厂商自有许可证）→ **完全开源**（权重 + 数据 + 训练代码齐备、OSI 认可的许可证，实际极少）。日常说的"开源模型"绝大多数落在中间这段，本书统一用**开放权重**指它。对 SRE 有两个直接后果：① **许可证是注册表字段，不是法务细节**——同样叫"开源"，能否商用、能否用输出训别的模型、派生模型是否必须改名，差别很大（分级见 [深入 15 · §2.1](../深入/15-模型注册表与上线流程.md)）；② **权重开放 ≠ 可复现可审计**——拿不到训练数据，合规审计只能答到"线上跑的是哪个权重版本"，答不到"这条用户数据训练过吗"（对照 [深入 14 · §3](../深入/14-微调作为运维对象.md) 自训模型的责任边界）|
...
| 能力脚手架 vs 可靠性脚手架 | 架构师搭的东西按半衰期分两类：**能力脚手架**补模型此刻做不到的事（few-shot 补服从、"先结构化再推理"等编排、外挂 skill），随模型变强而作废，应从一开始就搭成可一把删除；**可靠性脚手架**约束模型再强也不该独自决定的事（权限 / 沙箱 / 预算 / 回滚 / 审计），模型越强越需要。分辨判据：模型明天聪明十倍，还需不需要它。是全书"何时不再适用"触发器的哲学根，详见 [第 13 章 · §1](../架构/03-架构师的决策框架.md) |
