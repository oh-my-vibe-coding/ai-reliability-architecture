...
> [!NOTE]
> **目标**：SRE 运维过推理集群，但训练集群完全是另一回事。这一章告诉你**训练集群和推理集群的区别、并行方式、checkpoint 经济学、常见事故**——让你能负责训练集群 ops 或者至少和 ML infra 团队对话。

---

## 0. 训练和推理集群的本质差别

如果你只运维过推理集群，第一次接手训练集群会反复被同一件事打懵——下面先用一个心智模型 + 一个真实事故把直觉建立起来，再讲机制。

### 0.1 心智模型转换：训练集群不是"一堆机器"，是"一台机器"

这是这一章最重要的一句话。把它记牢，剩下的 80% 内容都讲得通。

**推理集群里**：1024 张 GPU = **1024 台独立的小机器**。一台挂了，剩下 1023 台继续接请求，没事，监控报警归报警，业务还在跑。

**训练集群里**：1024 张 GPU = **一台 1024 卡的巨型机器**。任何一张卡掉链子（网络抖动、ECC 错误、温度过高），**整个集群的训练全部卡住**——因为每一步训练都需要 1024 张卡同步交换梯度，缺一不可。

把这个事实记牢，下面所有"看起来过分"的设计就都讲得通了：

- **为什么必须 InfiniBand 不能用以太网**？因为 1024 张卡每一步要同步**几百 GB 数据**，普通以太网跟不上——只要有一根管子细，全集群被它拖
- **为什么 checkpoint 是生命线**？因为节点挂了不能"重试请求"，必须**整个集群从最近 checkpoint 重启**——所以 checkpoint 越新损失越小
- **为什么扩缩容这么手动**？因为加一张卡 = 改变并行拓扑 = 改变所有卡的协作方式，不是 K8s 加个 replica 就完事
- **为什么作业一跑就是数周不能停**？因为模型大、数据多、GPU 利用率高（60–70% MFU），中途停下来重启的代价巨大

### 0.2 一个真实事故：所有指标绿，但训练慢了 24 倍

ML 团队的 70B 继续预训练已经跑了 9 天，还要 19 天。凌晨 3 点告警：训练 step 速度从 **1.2 step/s 跌到 0.05 step/s**——慢了 24 倍，但**没崩**。
...
「**没坏啊？**」——这是推理 SRE 训练有素的反应。

ML 团队看了 10 秒就说：「NCCL all-reduce 卡住了，一定是某张卡的 IB 链路降速。`ib_write_bw` 测一下。」

果然，一台机器的 IB 端口从 **400 Gbps 跌到了 50 Gbps**（链路接触不良）。**就这一台**。但 1024 张卡每一步都得等它，所以**全集群被这一根管子拖慢了 24 倍**。

这是训练集群典型的事故形态：

- 故障**不局部**（一卡慢 = 全集群慢）
- 表象**像健康**（传统 CPU / Mem / HTTP / 磁盘全绿）
- 必须看**集合通信指标**（NCCL all-reduce 耗时、IB 带宽 / 错误计数、链路降速事件）

> 如果 SRE 没有 §0.1 那个心智模型，看着满屏绿灯找不到根因——只能干等 ML 团队来诊断。等 ML 团队到岗一看就知道是 IB，但**这种知识不应该让 ML 团队垄断**——SRE 只要懂一点，就能把这种事故的 MTTR 从小时级压到分钟级。

### 0.3 归纳：训练 vs 推理的差别（一张表）

把 §0.1 的心智模型和 §0.2 的事故落到一张对比表：
...
> [!WARNING]
> 用运维推理集群的思路去跑训练，**必然出大问题**。扩缩容、负载均衡、容错模型都不一样——上面那个 IB 事故就是一个具象化案例。

剩下的内容回答四个问题：
- 训练集群长什么形状？为什么必须 InfiniBand？（§1）
- 五种并行策略怎么选？（§2）
- Checkpoint 为什么是生命线、怎么算它的成本？（§3）
- 常见事故、调度、监控怎么做？（§4–§6）
...
图里的 **Pod / SU**（Scalable Unit，可扩展单元）是集群组网与调度的基本单元：64 台 8 卡节点装不进一个机柜，它们跨多个机柜组成一组，组内用 InfiniBand 全带宽互联，组与组之间再经上层交换机相连。
...
**InfiniBand** 单端口 400 Gbps（NDR），H100 节点通常每张 GPU 配一张 NIC，8 卡节点聚合 3.2 Tbps；延迟 <1μs。贵但不可或缺。
...
- Queue + partition 概念
- 适合大作业
- 原生不围绕容器设计，但可通过 Pyxis/Enroot（NVIDIA 默认栈）或 Apptainer 跑容器作业
