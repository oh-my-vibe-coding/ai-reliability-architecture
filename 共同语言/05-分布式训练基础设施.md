...
> [!WARNING]
> 用运维推理集群的思路跑训练，**必然爆炸**。扩缩容、负载均衡、容错模型都不一样。
...
> [!IMPORTANT]
> 训练集群的 ops 原则和推理**根本不同**——全集群一个故障域、InfiniBand 不可缺、Checkpoint 是生命线、Loss spike 有独特处理流。
>
> SRE 懂这些后，能从"买卡的"变成"ML infra 架构师"——这是 LLM 时代 SRE 最值钱的演进方向之一。
