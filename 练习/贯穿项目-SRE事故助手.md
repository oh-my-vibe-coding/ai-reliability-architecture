...
>
> 它的价值不在"跑起来"，在于让你**把 Unit 0-5 学的每一样东西落到一个真实系统上**。它是这本书的训练载体，不是这本书范围的全部——你会借它把 AgentOps、Trace-Eval、SLO、预算治理、Verifier、灰度回滚与事故模式映射串成一条完整的可靠性架构练习链路。
...
- **对 SRE 架构师，RAG + Tool-use Agent 比"调参小模型"更接近主战场**：你日常会运维别人训的模型、做架构决策、管 token 成本、设计权限边界，也会逐步碰到网关、模型服务、回滚与评估这些系统层问题，几乎不会自己去预训练一个新模型。
...
| LLM API | Anthropic / OpenAI | 本地 vLLM / Ollama（Unit 5 [本地部署路线 A/B](Unit5-数值与编译器级调试/总览.md#不同硬件条件的学习路线) 可选）|
...
> [!TIP]
> **不要在选型上纠结**。起步选**最小组合**（CLI + SQLite + Anthropic），要暴露 HTTP 接口时再换 FastAPI，其余跑起来再考虑替换。选型本身不是学习目标。
...
| Unit | 耗时 | 新增能力 | 交付物（累积）|
|---|---|---|---|
| **Unit 0** | 2 周 | 最小 API + RAG + tool use | 能跑的 CLI；20+ 文档 ingested |
| **Unit 1** | 4 周 | 权限、致命三角、红队 | 工具白名单；injection 测试报告 |
| **Unit 2** | 4 周 | Trace-Eval 一体化、三层 eval | trace schema；eval pipeline；失败回流 |
| **Unit 3** | 4 周 | SLO、容量、缓存、成本、灰度 | SLI 面板；决策树；灰度配置 |
| **Unit 4** | 3 周 | Step 预算、verifier | step 图；端到端正确率对比 |
| **Unit 5** | 2 周 | 数值级 runbook（+ 可选本地/量化） | 数值 runbook；本地部署验证（可选） |
| **Capstone** | 1 周 | 可靠性架构评审包 | 15 节完整评审文档 |

**核心设计**：每个 Unit 的**每周 B2 产出**尽量落到项目里——不是"写一份独立 markdown"，而是**在项目里新增代码 / 文档 / 配置**。
...
- 可选（路线 A/B）：`app/agent.py` 支持切换到 **本地小模型**（Qwen3-1.7B / Phi-4-mini；型号为 2026-07 快照，使用时以当时最新型号为准），对比质量 / 成本
...
### Capstone · AI 可靠性架构评审包（~1 周）

**不再新增代码**——直接把现有项目**沉淀为[架构评审文档](Capstone-AI生产架构评审包.md)**：

- 15 节评审包的所有内容**都已经在项目里**（docs/ + 代码 + 测试）
...
git check-ignore -q .env || echo "危险：.env 未被 git 忽略，先修 .gitignore 再提交"
mkdir -p app data/runbooks data/eval traces tests/injection docs
cp ../AI时代SRE架构师之路/代码/02-minimal-agent.py app/agent.py  # 假设本书仓库与项目同级，路径按你的实际位置调整
```
...
参考 [深入 09 · 何时不该用 AI](../深入/09-何时不该用AI.md) 的判断框架，把同一判断逻辑**往下再推一层**："该用 RAG + prompt"的理由，多数也是"不该微调"的理由。
...
**有**。SRE 未来运维的系统里**一定会有 LLM**。这个项目给你的是**带着 SRE 视角理解 AI 系统**的肌肉记忆——即使两年内你不碰 AI 线，当业务方要上 AI 时，你会是团队里**少数能把风险、成本、边界说清楚的人**。

### Q2：我的公司不让我用 Claude / GPT，能做吗？
**能**。全书的设计就是 vendor-neutral（见[深入 03](../深入/03-模型与工具场景化最佳实践.md)）。用**本地 vLLM + 开源模型**（Llama / Qwen）完全够用，只是 Unit 0 的 tool use 部分需要选**tool use 支持好的模型**（Qwen3-8B 或更大；型号为 2026-07 快照，使用时以当时最新型号为准）。
...
>
> **两者结合 = 4.5 个月把你从"SRE"变成"有 AI SRE 实战经验的 SRE"**。
>
