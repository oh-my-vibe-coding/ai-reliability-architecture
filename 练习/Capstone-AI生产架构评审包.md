...
**A/B/C/D 评级参考**——自 D 向上逐级排除：命中任一 D 判据即 D；否则命中任一 C 判据即 C；否则同时满足 A 全部判据才是 A；其余都是 B。任何系统都有唯一落点：

| 等级 | 判据 | 含义 |
|---|---|---|
| **D** | 命中任一：Forbidden 边界缺失 / 致命三角未砍腿 / 无 eval 覆盖 | **不该上线**，回到设计阶段 |
| **C** | 命中任一：多个 SLO 长期 burn 或质量分桶塌陷 / 红队 ≥1 critical 未关 / 0 verifier | 必须按 P0 改进，否则下调自治档 |
| **B** | 未落 C/D，但未同时满足 A 的全部判据（如个别 SLO 偶有 burn、红队仍有 high 未关、verifier 只有 1-2 个） | 维持现状，按 P1 节奏改进 |
| **A** | 同时满足：全部 SLO 达标 + 红队 0 critical 未关 + ≥3 个正交 verifier + Forbidden 边界完整 | 可继续扩自治 |
...
评审包写完后，先依次走前两轮；第三轮对应 Mastery Gate 的同事反馈项。
...
- **30 分钟**：逐节 challenge，聚焦 § 2、5、8、11（风险密集区）
...
[← 周循环总览](周循环总览.md)  ·  [📖 总目录](../README.md)  ·  [结语 →](../99-结语.md)
